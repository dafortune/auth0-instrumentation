...
module.exports = {
  express: function(tracer) {
    ...
    mw.wrap = (name, options, target) => {
      if (typeof options === 'function') {
        target = options;
        options = {};
      }

      const wrapped = (parentCtx, req, res, next) => {
        // Many times we use a middleware in a manual fashion
        // not as `express` middleware, or even in a mixed
        // way, this allows some flexibility in that regard while at the same
        // time allowing proper instrumentation
        if (!parentCtx.isTracingSpan) {
          req = parentCtx;
          res = req;
          next = res;
          parentCtx = req.a0trace ? req.a0trace.span : undefined;
        }
        ...
        try {
          if (options.exposeSpan) {
            target(span, req, res, wrappedNext);
          } else {
